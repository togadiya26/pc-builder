...
export default function UpdateAccessories(props) {
  ...
  const handleClickOpen = () => {
    ...
    if (props.sP.length !== 0 && props.index !== undefined) {
      setAddProduct({
        ...
        productditeils: props.sP[props.index]?.productditeils || "",
      });
      setDisabled(true)
    }
  };
  ...
  const Validation = () => {
    if (addProduct.productname === '') {
      ...
      return false;
    } else if (addProduct.productditeils === null) {
    ...
    if (Validation()) {
      try {
        const response = await axios.put(
          `https://pc-builder-backend-git-main-togadiya123.vercel.app/item/updateaccessories/${props.id}`, dataToSend, {
          headers: {
    ...
    <div>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent sx={{ backgroundColor: "aliceblue" }} >
          ...
          <TextField placeholder='product details' name="productditeils" value={addProduct.productditeils} onChange={handleInputChange} sx={{ width: "100%", marginBottom: "2%" }} />
