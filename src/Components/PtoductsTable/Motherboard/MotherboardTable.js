...
import style from './MotherboardTable.module.css';
...
import UpdateMotherboard from './UpdateMotherboard';
import AddMotherboard from './AddMotherboard';
import { getMotherboard } from '../../API/Api';
import ColorRingLoader from '../../Loader/ColorRingLoader';
import AddBulkMotherboard from './AddBulkMotherboard';
...
export default function MotherboardTable(props) {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Motherboard.length) : 0;
  ...
  return (
    <React.Fragment>
      <div className={style.tableHead}>
        ...
        <AddBulkMotherboard sSP={setMotherboard} load={load} />
        <AddMotherboard sP={Motherboard} sSP={setMotherboard} load={load} />
      </div>
      <div>
        <TableContainer component={Paper} sx={{ marginTop: "2%", backgroundColor: "aliceblue" }}>
          <Table sx={{ width: "100%" }} aria-label="custom pagination table">
            ...
            {load ?
              ...
              <TableBody>
                {
                  (rowsPerPage > 0 && Motherboard.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter((ele) => ele.productname.toLowerCase().toString().match(search.toLowerCase().toString()) ||
