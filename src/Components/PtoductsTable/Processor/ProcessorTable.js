...
import style from './ProcessorTable.module.css';
...
export default function ProcessorTable(props) {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Processor.length) : 0;
  ...
  const handleDelete = async (id, index) => {
    ...
    try {
      await axios.delete(`http://pc-builder-backend-git-main-togadiya123.vercel.app/item/deleteitem/processors/${id}`, {
        headers: {
      ...
      <div>
        <TableContainer component={Paper} sx={{ marginTop: "2%", backgroundColor: "linen" }}>
          <Table sx={{ width: "100%" }} aria-label="custom pagination table">
            ...
            <TableBody>
              {
              (rowsPerPage > 0 && Processor.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter((ele) => ele.productname.toLowerCase().toString().match(search.toLowerCase().toString()) ||
