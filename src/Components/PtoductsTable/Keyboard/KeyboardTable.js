...
import style from './KeyboardTable.module.css';
...
export default function KeyboardTable(props) {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Keyboard.length) : 0;
  ...
  const handleDelete = async (id, index) => {
    ...
    try {
      await axios.delete(`http://pc-builder-backend-git-main-togadiya123.vercel.app/item/deleteitem/keyboards/${id}`, {
        headers: {
        ...
        <TableContainer component={Paper} sx={{ marginTop: "2%", backgroundColor: "linen" }}>
          <Table sx={{ width: "100%" }} aria-label="custom pagination table">
            ...
            {load ?
              ...
              <TableBody>
                {
                  (rowsPerPage > 0 && Keyboard.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter((ele) => ele.productname.toLowerCase().toString().match(search.toLowerCase().toString()) ||
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell style={{ width: "20%" }} align="center">
                        <div className={style.buttonCell}>
                          <UpdateKeyboard id={row._id} index={index} sP={Keyboard} sSP={setKeyboard} />
                          <Button
