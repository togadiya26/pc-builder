...
import style from './MouseTable.module.css';
...
export default function MouseTable(props) {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Mouse.length) : 0;
  ...
  return (
    <React.Fragment>
      <div className={style.tableHead}>
        ...
        <AddBulkMouse sSP={setMouse} load={load} />
        <AddMouse sP={Mouse} sSP={setMouse} load={load} />
      </div>
      <div>
        <TableContainer component={Paper} sx={{ marginTop: "2%", backgroundColor: "aliceblue" }}>
          <Table sx={{ width: "100%" }} aria-label="custom pagination table">
            ...
            {load ?
              ...
              <TableBody>
                {
                  (rowsPerPage > 0 && Mouse.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter((ele) => ele.productname.toLowerCase().toString().match(search.toLowerCase().toString()) ||
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell style={{ width: "20%" }} align="center">
                        <div className={style.buttonCell}>
                          <UpdateMouse id={row._id} index={index} sP={Mouse} sSP={setMouse} />
                          <Button
