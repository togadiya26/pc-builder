...
import style from './PowerSupplyUnitTable.module.css';
...
import AddPowersupplyunit from './AddPowerSupplyUnit';
import UpdatePowersupplyunit from './UpdatePowerSupplyUnit';
import ColorRingLoader from '../../Loader/ColorRingLoader';
import AddBulkPowerSupplyUnit from './AddBulkPowerSupplyUnit';
...
export default function PowerSupplyUnitTable(props) {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - PowerSupplyUnit.length) : 0;
  ...
  return (
    <React.Fragment>
      <div className={style.tableHead}>
        ...
        <AddBulkPowerSupplyUnit sSP={setPowerSupplyUnit} load={load} />
        <AddPowersupplyunit sP={PowerSupplyUnit} sSP={setPowerSupplyUnit} load={load} />
      </div>
      <div>
        <TableContainer component={Paper} sx={{ marginTop: "2%", backgroundColor: "aliceblue" }}>
          <Table sx={{ width: "100%" }} aria-label="custom pagination table">
            ...
            {load ?
              ...
              <TableBody>
                {
                  (rowsPerPage > 0 && PowerSupplyUnit.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter((ele) => ele.productname.toLowerCase().toString().match(search.toLowerCase().toString()) ||
