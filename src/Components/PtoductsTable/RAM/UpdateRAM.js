...
export default function UpdateRam(props) {
  ...
  const handleClickOpen = () => {
    ...
    if (props.sP.length !== 0 && props.index !== undefined) {
      setAddProduct({
        ...
        ramcapacity: props.sP[props.index]?.ramcapacity || "",
      });
      setDisabled(true)
    }
  };
  ...
  const Validation = () => {
    if (addProduct.productname === '') {
      ...
      return false;
    } else if (addProduct.ramcapacity === null) {
    ...
    if (Validation()) {
      try {
        const response = await axios.put(
          `https://pc-builder-backend-git-main-togadiya123.vercel.app/item/updateram/${props.id}`, dataToSend, {
          headers: {
        ...
        console.log(error);
        alert("Error occurred while updating Ram.");
      }
    ...
    <div>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent sx={{ backgroundColor: "aliceblue" }} >
          ...
          <TextField placeholder='ramcapacity' name="ramcapacity" value={addProduct.ramcapacity} onChange={handleInputChange} sx={{ width: "100%", marginBottom: "2%" }} />
