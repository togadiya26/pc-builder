...
import style from './RamTable.module.css';
...
import AddBulkRam from './AddBulkRam';
...
export default function RamTable(props) {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - RAM.length) : 0;
  ...
  return (
    <React.Fragment>
      <div className={style.tableHead}>
        ...
        <AddBulkRam sSP={setRAM} load={load} />
        <AddRam sP={RAM} sSP={setRAM} load={load} />
      </div>
      <div>
        <TableContainer component={Paper} sx={{ marginTop: "2%", backgroundColor: "aliceblue" }}>
          <Table sx={{ width: "100%" }} aria-label="custom pagination table">
            ...
            {load ?
              ...
              <TableBody>
                {
                  (rowsPerPage > 0 && RAM.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter((ele) => ele.productname.toLowerCase().toString().match(search.toLowerCase().toString()) ||
