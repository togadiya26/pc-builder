...
import style from './Storage1Table.module.css';
...
export default function Storage1Table(props) {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Storage1.length) : 0;
  ...
  const handleDelete = async (id, index) => {
    ...
    try {
      await axios.delete(`http://pc-builder-backend-git-main-togadiya123.vercel.app/item/deleteitem/storage1/${id}`, {
        headers: {
        ...
        <TableContainer component={Paper} sx={{ marginTop: "2%", backgroundColor: "linen" }}>
          <Table sx={{ width: "100%" }} aria-label="custom pagination table">
            ...
            {load ?
              ...
              <TableBody>
                {
                  (rowsPerPage > 0 && Storage1.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter((ele) => ele.productname.toLowerCase().toString().match(search.toLowerCase().toString()) ||
