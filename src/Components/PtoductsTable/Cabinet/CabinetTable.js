...
import style from './CabinetTable.module.css';
...
export default function CabinetTable(props) {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Cabinet.length) : 0;
  ...
  const handleDelete = async (id, index) => {
    ...
    try {
      await axios.delete(`http://pc-builder-backend-git-main-togadiya123.vercel.app/item/deleteitem/cabinets/${id}`, {
        headers: {
      ...
      {load ?
        ...
        <>
          <div className={style.tableHead}>
            ...
            <AddCabinet sP={Cabinet} sSP={setCabinet} />
          </div>
          <div>
            <TableContainer component={Paper} sx={{ marginTop: "2%", backgroundColor: "linen" }}>
              <Table sx={{ width: "100%" }} aria-label="custom pagination table">
                ...
                <TableBody>
                  {
                    (rowsPerPage > 0 && Cabinet.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter((ele) => ele.productname.toLowerCase().toString().match(search.toLowerCase().toString()) ||
