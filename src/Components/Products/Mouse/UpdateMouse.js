...
export default function UpdateMouse(props) {
  ...
  const handleClickOpen = () => {
    ...
    if (props.sP.length !== 0 && props.index !== undefined) {
      setAddProduct({
        ...
        size: props.sP[props.index]?.size || "",
        connectivity: props.sP[props.index]?.connectivity || ""
      });
      setDisabled(true)
    }
  };
    ...
    if (Validation()) {
      try {
        const response = await axios.put(
          `https://pc-builder-backend-git-main-togadiya123.vercel.app/item/updatemouse/${props.id}`, addProduct, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
        console.log(response);
        alert("Mouse Updated successfully!");
      } catch (error) {
        console.log(error);
  ...
  function handleImageChange(event) {
    ...
    const storageRef = ref(storage, `/Mouse/${fileInput.files[0].name}`);
    const uploadTask = uploadBytesResumable(storageRef, fileInput.files[0]);

    // Mouse the upload progresssize
