...
export default function UpdateStorage2(props) {
    ...
    if (Validation()) {
      try {
        const response = await axios.put(
          `https://pc-builder-backend-git-main-togadiya123.vercel.app/item/updatestorage2/${props.id}`, addProduct, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
        console.log(response);
        alert("Storage2 Updated successfully!");
      } catch (error) {
        console.log(error);
        alert("Error occurred while updating Storage2.");
      }
  ...
  function handleImageChange(event) {
    ...
    const storageRef = ref(storage, `/Storage2/${fileInput.files[0].name}`);
    const uploadTask = uploadBytesResumable(storageRef, fileInput.files[0]);

    // Storage2 the upload progresscapacity
