...
export default function AddCabinetFan(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (Validation()) {
      ...
      const storageRef = ref(storage, `/Cabinet Fan/${fileInput.files[0].name}`);
      ...
      uploadTask.on("state_changed", (snapshot) => {
        ...
        try {
          const response = await axios.post(
            "https://pc-biult-backend-git-main-togadiya123.vercel.app/api/user/addcabinetfan",
            {
              // name: name,
